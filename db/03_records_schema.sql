...
--   • JWT payload: { user_id, name, grade, center, iat, exp }
--   • 학생 브라우저 → Supabase 직접 호출 시 JWT 첨부 (Authorization: Bearer)
--   • Supabase는 RLS에서 JWT의 user_id 검증
--
-- 적용 순서:
...
CREATE TABLE user_profile_cache (
  external_user_id  TEXT PRIMARY KEY,            -- 구서버 회원 ID (JWT의 user_id)
...
CREATE TABLE attempts (
  ...
  external_user_id  TEXT NOT NULL,                                 -- 구서버 회원 ID
...
CREATE INDEX idx_attempts_user_session ON attempts (external_user_id, session_id, finished_at DESC);
CREATE INDEX idx_attempts_user_area    ON attempts (external_user_id, area_id);
...
CREATE TABLE word_results (
  ...
  external_user_id  TEXT NOT NULL,                                 -- 비정규화 (자주 조회)
...
CREATE INDEX idx_word_results_user_word  ON word_results (external_user_id, word_master_id, answered_at DESC);
...
CREATE TABLE area_progress (
  ...
  PRIMARY KEY (external_user_id, area_id)
);
...
BEGIN
  ...
  IF (TG_OP = 'DELETE') THEN
    v_user := OLD.external_user_id;
    v_area := OLD.area_id;
  ELSE
    v_user := NEW.external_user_id;
    v_area := NEW.area_id;
  END IF;

  INSERT INTO area_progress (external_user_id, area_id, cleared_count, total_attempts, last_session_id, last_attempt_at)
  SELECT
    v_user,
    v_area,
    ...
    (SELECT session_id FROM attempts
       WHERE external_user_id = v_user AND area_id = v_area
       ORDER BY COALESCE(finished_at, started_at) DESC LIMIT 1),
    MAX(COALESCE(a.finished_at, a.started_at))
  FROM attempts a
  WHERE a.external_user_id = v_user AND a.area_id = v_area
  ON CONFLICT (external_user_id, area_id) DO UPDATE SET
...
--   • Supabase는 JWT의 sub 또는 커스텀 클레임을 request.jwt.claims로 노출
--   • 우리는 user_id 클레임을 external_user_id에 매핑
...
CREATE POLICY "own_select" ON user_profile_cache
  FOR SELECT USING (external_user_id = current_setting('request.jwt.claims', true)::json->>'user_id');
...
CREATE POLICY "own_insert" ON attempts
  FOR INSERT WITH CHECK (external_user_id = current_setting('request.jwt.claims', true)::json->>'user_id');
...
CREATE POLICY "own_update" ON attempts
  FOR UPDATE USING (external_user_id = current_setting('request.jwt.claims', true)::json->>'user_id');
...
--   • 영역별 통과 회차 수 기준 내림차순
--   • 이름·센터·학년은 join해서 표시 (RLS 우회 위해 SECURITY DEFINER 함수 권장 — 일단 view로)
-- ════════════════════════════════════════
...
LEFT JOIN user_profile_cache upc ON upc.external_user_id = ap.external_user_id
WHERE ap.cleared_count > 0;

COMMENT ON VIEW v_area_ranking IS '영역별 랭킹 — 통과 회차 수 기준. 클라이언트는 area_id 필터 + LIMIT 100';

-- 랭킹 view는 누구나 읽기 가능 (이름·센터는 게임 특성상 공개 OK)
-- RLS는 base table에서 막히지만 view는 SECURITY DEFINER 모드로 우회 필요 시 별도 함수 작성
