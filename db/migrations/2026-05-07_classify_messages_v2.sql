-- ════════════════════════════════════════════════════════════════
-- Migration: classify_word_input 메시지 개선 v2
-- Date: 2026-05-07
--
-- 변경:
--   • "가족 어휘를 찾았어요!" → "'기생'을(를) 찾았어요!" (입력 단어 + 받침 자동 을/를)
--   • "한자어이지만 이번 회차의 가족 어휘가 아니에요." → "어려운 한자어를 알고 있네요. 훌륭해요!"
--
-- 적용:
...
BEGIN
  ...
  -- 2단계: 표준국어대사전·우리말샘에서 主字 포함 한자어 매칭 → 칭찬
...
-- 검증
SELECT
  classify_word_input('기생', '生') AS test_correct_with_받침,
  classify_word_input('학교', '校') AS test_correct_no_받침,
  classify_word_input('야생', '生') AS test_correct_야,
